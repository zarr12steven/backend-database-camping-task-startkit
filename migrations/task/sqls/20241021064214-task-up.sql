...
SELECT
    ...
    COALESCE(tp.total_purchased, 0) - COALESCE(tu.total_used, 0) AS remaining_credits
FROM "USER" u
LEFT JOIN total_purchased tp ON u.id = tp.user_id
LEFT JOIN total_used tu ON u.id = tu.user_id
WHERE u.name = '王小明';
